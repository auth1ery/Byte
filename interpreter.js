const RADIO_TRACKS = [
  { title: "telecaster (ft. palm) — hazerred", url: "https://files.catbox.moe/wuojo2.mp3" },
  { title: "STRIKE OF LUCK — LEONJD", url: "https://files.catbox.moe/c19q4q.mp3" },
  { title: "core — hazerred", url: "https://files.catbox.moe/zq2xpc.mp3" },
  { title: "CRASHPARTY — azalea", url: "https://files.catbox.moe/46zyfi.mp3" },
  { title: "Alone Inteligence - Camellia", url: "https://files.catbox.moe/7rotja.mp3" },
  { title: "So Far So Fake - Pierce The Veil", url: "https://files.catbox.moe/y0mwqb.mp3" },
  { title: "webpunk (ft. nekosnicker) - vylet pony", url: "https://files.catbox.moe/5iqb7s.mp3" },
  { title: "questions - tristam", url: "https://files.catbox.moe/7h667b.mp3" },
  { title: "synthmind - spade", url: "https://files.catbox.moe/e5ocj3.mp3" },
  { title: "go long - hazerred", url: "https://files.catbox.moe/qrw3dd.mp3" },
  { title: "GLORY：ROAD - uma and Morimori Atsushi", url: "https://files.catbox.moe/361d1d.mp3" },
  { title: "Ether Strike ('Divine Mercy' Extended) - Akira Complex", url: "https://files.catbox.moe/6t44ld.mp3" },
  { title: "Melon - Chiru-san", url: "https://files.catbox.moe/87hkdx.mp3" },
  { title: "wonder - auth", url: "https://files.catbox.moe/q9t1sb.mp3" },
  { title: "crystallized - camellia", url: "https://files.catbox.moe/9ycbll.mp3" },
  { title: "children of the city - mili", url: "https://files.catbox.moe/hwy10v.mp3" },
  { title: "soar - chiru-san", url: "https://files.catbox.moe/qma461.mp3" },
  { title: "death & romance - magdalena bay", url: "https://files.catbox.moe/p0144j.mp3" },
  { title: "daydreamer - SPURME and Enkei", url: "https://files.catbox.moe/yx8x9l.mp3" },
  { title: "moment - vierre cloud", url: "https://files.catbox.moe/8q3dsn.mp3" },
  { title: "light/speed - candle", url: "https://files.catbox.moe/ia0k7r.mp3" },
  { title: "a new kind of love - frou frou", url: "https://files.catbox.moe/8nhoab.mp3" }
];
...
(async () => {
  ...
  async function fetchDiagnostics() {
    ...
    console.log(`@${t} ${regs.join(" ")} | ${cpuFlags} | heap:${heap.join(" ")} | stack:[${callStack.join("->")}] | memory:${memMap.join(" ")}`);
  }
...
function logToIDE(msg) {
  const out = document.getElementById("output");
  if (out) {
    out.textContent += msg + "\n"; // append to IDE console
    out.scrollTop = out.scrollHeight; // auto-scroll to bottom
  } else {
    console.warn("[logToIDE] Output element not found:", msg);
  }
}
...
function findSong(query) {
  ...
  return RADIO_TRACKS.find(t => t.title.toLowerCase().includes(query));
}

function getPlayer() {
  if (!__byte_runtime.player) {
    __byte_runtime.player = new Audio();
    __byte_runtime.player.volume = 0.7;
  }
  return __byte_runtime.player;
}
...
const libraries = {
  music: {
    play: (query) => {
      ...
      if (!song) {
        logToIDE(`[music] Song not found: ${query}`);
        return;
      }
      const player = getPlayer();
      player.src = song.url;
      player.play();
      logToIDE(`[music] Now playing: ${song.title}`);
    },
    stop: () => {
      const player = getPlayer();
      player.pause();
      player.currentTime = 0;
      logToIDE(`[music] Stopped`);
    },
    pause: () => {
      getPlayer().pause();
      logToIDE(`[music] Paused`);
    },
    resume: () => {
      getPlayer().play();
      logToIDE(`[music] Resumed`);
    }
  }
};

// active libraries (like imports)
let activeLibraries = {};

// ===============================
// Library management helper
// ===============================
function extractLibrary(libName) {
  const name = libName.toLowerCase();
  if (libraries[name]) {
    activeLibraries[name] = libraries[name];
    logToIDE(`[system] Library loaded: ${name}`);
  } else {
    logToIDE(`[system] No such library: ${name}`);
  }
}
...
const BYTE_KEYWORDS = [
  ...
  "ST","AT","TA","BT","TS", "math.Add", "math.Sub", "math.Mul", "math.Div", "cringe", "EXTRACT", "PLAY", "STOP", "PAUSE", "RESUME"
...
// probably this auth dude idk lmao auth rpobably broke the shit out of it
// or this guy named jx1dac ths
...
// goo goo gaga
  ...
  // math, boo boring math sucks boo
...
// ===============================
// Library management & Music commands (safe, i hope, idk))
// ===============================
if (line.toUpperCase().startsWith("EXTRACT ")) {
  const libName = line.slice(8).trim().toLowerCase();
  extractLibrary(libName);
  return;
}

if (line.toUpperCase().startsWith("PLAY ")) {
  const query = line.slice(5).trim();
  if (activeLibraries.music?.play) {
    activeLibraries.music.play(query);
  } else {
    logToIDE(`[error] Music library not loaded. Use: EXTRACT music`);
  }
  return;
}

if (line.toUpperCase() === "STOP") {
  if (activeLibraries.music?.stop) {
    activeLibraries.music.stop();
  } else {
    logToIDE(`[error] Music library not loaded. Use: EXTRACT music`);
  }
  return;
...
if (line.toUpperCase() === "PAUSE") {
  if (activeLibraries.music?.pause) {
    activeLibraries.music.pause();
  } else {
    logToIDE(`[error] Music library not loaded. Use: EXTRACT music`);
  }
  return;
...
if (line.toUpperCase() === "RESUME") {
  if (activeLibraries.music?.resume) {
    activeLibraries.music.resume();
  } else {
    logToIDE(`[error] Music library not loaded. Use: EXTRACT music`);
  }
  return;
...
document.getElementById("docs").addEventListener("click", ()=>{
  window.open("https://bytewiki.miraheze.org/wiki/Main_Page", "_blank");
});
