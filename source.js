...
const BYTE_KEYWORDS=["STR","EXE","RED","DLE","VAR","STO","THN","WIT","LIN","RRI","onStr","func","const","AND","NOT","IF","Change","SIPHON","Share","addNum","dleNum","Do","-c","Button","Check","Spawn.New","raycast","Hit","Keybind","velocity","require","ST","AT","TA","BT","TS","math.Add","math.Sub","math.Mul","math.Div","EXTRACT","PLAY","STOP","PAUSE","RESUME","style","anim","color","pos"].map(k=>k.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'));
...
async function runByteAsync(code){
 ...
 function precheckBlocks(q){const stack=[];const errors=[];for(let i=0;i<q.length;i++){const ln=q[i].line;if(!ln||ln.startsWith("//")||ln.startsWith("=//"))continue;if(/^func\s+[A-Za-z_]\w*\s*=$/i.test(ln)||/^[A-Za-z_]\w*\s*=$/.test(ln)){stack.push({type:"func",line:i});continue}if(/^-c\s*\+\s*\d+\s*=$/.test(ln)){stack.push({type:"loop",line:i});continue}if(ln==="-"){const top=stack.pop();if(!top||top.type!=="loop")errors.push({msg:'Unmatched loop close "-"',line:i});continue}if(/^end$/i.test(ln)||/^END$/.test(ln)){const top=stack.pop();if(!top||top.type!=="func")errors.push({msg:'Unmatched END / end',line:i});continue}}while(stack.length){const t=stack.pop();errors.push({msg:"Unclosed block: "+t.type,line:t.line})}return errors}
 const preErrors=precheckBlocks(queue);if(preErrors.length){const first=preErrors[0];highlightError(first.line);const messages=preErrors.map(e=>`Line ${e.line+1}: ${e.msg}`).join("\n");const outEl=document.getElementById("output");if(outEl)outEl.textContent="Parse Errors:\n"+messages;else console.error("Parse Errors:\n"+messages);return}
 ...
 function safeEval(expr,locals){const transformed=transformExprToJS(expr);if(!/^[\w\s\+\-\*\/\.\(\)\<\>\!\=\&\|\?"',:]+$/.test(transformed))throw new Error("Unsafe characters in expression: "+expr);const replaced=transformed.replace(/\b([A-Za-z_]\w*)\b/g,(m)=>{if(m==="true"||m==="false")return m;if(/^\d+(\.\d+)?$/.test(m))return m;return `__V["${m}"]`});const __V=Object.assign({},globalVars,locals||{});const fn=new Function("__V","return ("+replaced+");");return fn(__V)}

for(let i=0;i<queue.length;i++){const line=queue[i].line;let m=line.match(/^func\s+([A-Za-z_]\w*)\s*=$/i);if(!m)m=line.match(/^([A-Za-z_]\w*)\s*=$/);if(m){const name=m[1];const body=[];let j=i+1;while(j<queue.length&&!/^end$/i.test(queue[j].line)&&queue[j].line!=="END"){body.push(queue[j]);j++}functions[name]=body;i=j}}

function parseConcatExpression(expr,locals){const parts=[];let cur="";let inQuotes=false;for(let i=0;i<expr.length;i++){const c=expr[i];if(c==='"'){cur+=c;inQuotes=!inQuotes;continue}if(!inQuotes&&c==='+'){parts.push(cur.trim());cur=""}else cur+=c}if(cur.trim()!=="")parts.push(cur.trim());const evaluated=parts.map(p=>{if(/^".*"$/.test(p))return p.slice(1,-1);if(isNumericString(p))return Number(p);try{if(/^[A-Za-z_]\w*$/.test(p)){const v=lookupVar(p,locals);return v}const val=safeEval(p,locals);return val}catch(err){return p}});return evaluated.join("")}

const callStack=[];
async function executeFunction(fnName){if(!functions[fnName])throw new Error("Function not found: "+fnName);callStack.push(fnName);const locals={};for(let i=0;i<functions[fnName].length;i++){await executeLine(functions[fnName][i],locals);if(stopped)break}callStack.pop()}
function collectBlock(startIndex){const block=[];let j=startIndex+1;while(j<queue.length&&queue[j].line!=="-"){block.push(queue[j]);j++}return{block,endIndex:j}}
function operateAddDle(op,amount,locals){let target=declaredVarOrder.length?declaredVarOrder[declaredVarOrder.length-1]:null;if(!target){const keys=Object.keys(globalVars);target=keys.length?keys[0]:null}if(!target)throw new Error(op+" has no target variable to operate on.");const curr=lookupVar(target,locals);const newVal=(Number(curr)||0)+(op==="add"?Number(amount):-Number(amount));if(locals&&!Object.prototype.hasOwnProperty.call(globalVars,target))locals[target]=newVal;else globalVars[target]=newVal}

async function executeLine(lineObj,locals={}){
 const line=lineObj.line,number=lineObj.number;if(!line||line.startsWith("//")||line.startsWith("=//")||line==="STR"||line==="-"||lineObj._skipped)return;highlightCurrentLine(number);try{
 let m;
 ...
 if(line.startsWith("EXE")){m=line.match(/^EXE\s*\[\s*([A-Za-z_]\w*)\s*\]\s*$/);if(!m)throw new Error("Invalid EXE syntax");await executeFunction(m[1]);return}
 ...
 if(line.startsWith("-c")){m=line.match(/^-c\s*\+\s*(\d+)\s*=$/);if(m){const count=Number(m[1]);const {block,endIndex}=collectBlock(number);for(let k=number+1;k<=endIndex;k++)if(queue[k])queue[k]._skipped=true;for(let iter=0;iter<count;iter++){for(let b=0;b<block.length;b++){await executeLine(block[b],locals);if(stopped)break}if(stopped)break}return}}
 if(m=line.match(/^-c\s*\+\s*(\d+)\s*=\s*(.+)-\s*$/)){const count=Number(m[1]);const cmd=m[2].trim();for(let i=0;i<count;i++){await executeLine({line:cmd,number},locals);if(stopped)break}return}
 ...
 if(line.startsWith("IF")){m=line.match(/^IF\s+(.+)\s+THN\s*$/);if(m){const expr=m[1].trim();const block=[];let j=number+1;while(j<queue.length&&!/^(END|end)$/.test(queue[j].line)){block.push(queue[j]);j++}if(safeEval(expr,locals)){for(let b=0;b<block.length;b++){await executeLine(block[b],locals);if(stopped)break}}return}}
 ...
 return}catch(err){highlightError(number);output.push(`Error on line ${number+1}: ${err.message}`);if(callStack.length>0)output.push("Stack trace:\n"+callStack.join(" ->"));throw err}}
